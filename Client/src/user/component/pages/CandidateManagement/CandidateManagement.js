...
function CandidateManagement({ language = 'vi' }) {
  ...
  const handleViewCv = useCallback((candidate) => {
    const cvSrc = candidate.cv_file_base64 || (candidate.cv_file_path ? `${API_BASE_URL}${candidate.cv_file_path.replace(/\\/g, '/')}` : '');
    if (!cvSrc) return;
  ...
  const renderCompareModal = () => {
    ...
    const cvSrc = compareCandidate.cv_file_base64 || (compareCandidate.cv_file_path ? `${API_BASE_URL}${compareCandidate.cv_file_path.replace(/\\/g, '/')}` : '');
              ...
              {candidate.cv_file_base64 || candidate.cv_file_path ? (
                <div className={cx('cand-card__cv-action')} style={{ display: 'flex', flexDirection: 'column', gap: '8px', width: '100%' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px', width: '100%' }}>
                    ...
                    <a
                      href={candidate.cv_file_base64 || `${API_BASE_URL}${candidate.cv_file_path.replace(/\\/g, '/')}`}
